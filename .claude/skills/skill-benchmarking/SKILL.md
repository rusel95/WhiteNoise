...
Creates `eval-<name>/eval_metadata.json` and empty output dirs for `<model-slug>-with` and `<model-slug>-without`.
...
After receiving the JSON array, save it to `/tmp/grades_SET_A.json` and ingest:
```bash
python .github/skills/skill-benchmarking/scripts/ingest-grades.py \
  /tmp/grades_SET_A.json \
  workspaces/<platform>/<skill-name>/iteration-N \
  <model-slug>
```
Repeat for the other variant (SET_B = without_skill).
