...
| --- | --- | --- | --- |
| **GPT-5.4** | 100% | 59.2% | **+40.8%** |
| **Gemini 3.1 Pro** | 98.6% | 22.5% | **+76.0%** |

### Tiered Results (GPT-5.4)

| Difficulty | With Skill | Without Skill | Delta |
| --- | --- | --- | --- |
| Simple | 22/22 (100%) | 22/22 (100%) | **0%** |
| Medium | 23/23 (100%) | 11/23 (47.8%) | **+52.2%** |
| Complex | 26/26 (100%) | 9/26 (34.6%) | **+65.4%** |
| **Total** | **71/71 (100%)** | **42/71 (59.2%)** | **+40.8%** |

### Tiered Results (Gemini 3.1 Pro)

| Difficulty | With Skill | Without Skill | Delta |
| --- | --- | --- | --- |
| Simple | 21/22 (95.5%) | 7/22 (31.8%) | **+63.6%** |
| Medium | 23/23 (100%) | 4/23 (17.4%) | **+82.6%** |
| Complex | 26/26 (100%) | 5/26 (19.2%) | **+80.8%** |
| **Total** | **70/71 (98.6%)** | **16/71 (22.5%)** | **+76.0%** |

**Interpretation:** The baseline already gets the straightforward SwiftUI MVVM prompts right, but medium and complex scenarios still need the skill's Apple-specific Observation guidance. The biggest misses without the skill are `@Bindable`, `.task(id:)`, Observation-native environment injection, keeping navigation state out of the ViewModel, and the performance architecture around focused `@Observable` models.

**Gemini 3.1 Pro note:** Largest delta of all tested skills and models (+76.0%). Without-skill baseline is only 22.5% — the lowest across all 4 skills. Gemini defaults to Combine-era patterns (`@StateObject`, `@EnvironmentObject`, `@ObservedObject`) without the skill and misses the entire `@Observable` paradigm. With skill, it scores 98.6% showing it can follow the guidance correctly. This skill is **essential** for Gemini on SwiftUI iOS 17+ code.
