#!/usr/bin/env python3
"""Generate koubou-framed App Store screenshots for all locales."""
...
# Screenshot IDs mapped to raw screenshot files and accent colors
SCREENSHOTS = {
    "mix-sounds": {"file": "01_main_grid_playing.png", "accent": "#00BCD4"},
    "sleep-timer": {"file": "02_timer_modal.png", "accent": "#7C4DFF"},
    "rain-variants": {"file": "03_variant_picker_rain.png", "accent": "#4FC3F7"},
}
...
def build_config(locale: str, texts: dict) -> dict:
    """Build a koubou YAML config for a single locale."""
    screenshots = {}
    for shot_id, meta in SCREENSHOTS.items():
        t = texts[shot_id]
        screenshots[shot_id] = {
            "content": [
                {
                ...
                {
                    "type": "image",
                    "asset": f"public/screenshots/{meta['file']}",
                    "frame": True,
                    "position": ["50%", "68%"],
                    "scale": 0.85,
                },
    ...
    return {
        "defaults": {
            "background": {
                "type": "linear",
                "colors": ["#090c14", "#0c1020", "#0a0e1a", "#080b15"],
                "direction": 180,
            }
        },
        "project": {
            ...
            "output_dir": f"output/framed/{locale}",
...
def main():
    ...
    locales_to_render = {target: LOCALES[target]} if target else LOCALES
    total = 0
    failed = []

    for locale, texts in locales_to_render.items():
        config = build_config(locale, texts)
        ...
        try:
            ...
            if result.returncode == 0:
                total += 3
                print(f"  ✓ [{locale}] 3 screenshots")
            else:
                failed.append(locale)
                print(f"  ✗ [{locale}] FAILED: {result.stderr[:200]}")
        except subprocess.TimeoutExpired:
            failed.append(locale)
            print(f"  ✗ [{locale}] TIMEOUT")
        finally:
            Path(config_path).unlink(missing_ok=True)

    print(f"\nGenerated {total} screenshots for {len(locales_to_render) - len(failed)} locale(s)")
    if failed:
        print(f"Failed locales: {', '.join(failed)}")
        sys.exit(1)
